...
def reset_connection():
    ...
    conn = psycopg2.connect(
        ...
        database="school_db",
        user="postgres",
        password="31"  # PostgreSQL şifrenizi yazın
    )
...
def add_student():
    ...
    try:
        ...
        sections = ['A', 'B', 'C', 'D']  # Sabit section listesi
        cur.close()
...
def edit_student(student_id):
    try:
        ...
        cur.execute("SELECT DISTINCT class_id, class_name FROM class")  # Burada DISTINCT eklendi
        classes = cur.fetchall()
        sections = ['A', 'B', 'C', 'D']
        cur.close()
...
# Teachers (Similar structure to students, can be implemented similarly)
# Classes, Subjects, Rooms CRUD operations follow the same structure
...
def add_teacher():
    try:
        ...
        # Tüm sınıf bilgilerini al
        cur.execute("SELECT class_id, class_name FROM class")
        classes = cur.fetchall()

        # Tüm konu bilgilerini al
...
def edit_teacher(teacher_id):
    try:
        ...
        # Mevcut öğretmen bilgilerini al
...
def add_subject():
    if request.method == 'POST':
        try:
            data = request.form
            cur = conn.cursor()
            cur.execute("INSERT INTO subject (name, class_id, teacher_id) VALUES (%s, %s, %s)", 
                        (data['name'], data['class_id'], data['teacher_id']))
            conn.commit()
            cur.close()
    ...
    try:
        ...
        teachers = cur.fetchall()
        cur.close()
        return render_template('add_subject.html', classes=classes, teachers=teachers)
    except psycopg2.Error as e:
        conn.rollback()
        return f"An error occurred: {e}"
